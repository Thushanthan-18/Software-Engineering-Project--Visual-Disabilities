 ...
 * 08/05/2002 Cursor (caret) rendering fixed for JDK 1.4 (Anonymous) You may use
 * and modify this package for any purpose. Redistribution is permitted, in both
 * source and binary form, provided that this notice remains intact in all
 * source distributions of this package.
 */

package utils.jsyntax;

import utils.jsyntax.tokenmarker.Token;
import utils.jsyntax.tokenmarker.TokenMarker;
...
public class TextAreaPainter extends JComponent implements TabExpander {
    ...
    public TextAreaPainter(JEditTextArea textArea, TextAreaDefaults defaults) {
        ...
        if (lineNumbers) {
            leftBorder = fm.charWidth('0') * 5; // The width of the line numbers
            // column
        } else {
            leftBorder = 0;
        }
    ...
    /**
     * Returns the width of the left column used for line numbers.
     */
    ...
    public interface Highlight {
         ...
         * highlighter doesn't know what to display, it should delegate to the next
         * highlight painter.
         *
    ...
    public void paint(Graphics gfx) {
        ...
        try {
            ...
            int x = textArea.getHorizontalOffset() + leftBorder;
    ...
    public float nextTabStop(float x, int tabOffset) {
        int offset = textArea.getHorizontalOffset() + leftBorder;
    ...
    protected int leftBorder; // Used to simplify the handling of the line numbers
    // column
    ...
    protected void paintLine(Graphics gfx, TokenMarker tokenMarker, int line,
        ...
        paintMargin(gfx, defaultFont, line, x, y);
        if (lineNumbers)
            paintLineNumber(gfx, defaultFont, line, x, y);
    ...
    protected void paintSyntaxLine(Graphics gfx, TokenMarker tokenMarker,
        ...
        gfx.setColor(defaultColor);
        y += fm.getHeight();
        x = SyntaxUtilities.paintSyntaxLine(currentLine, currentLineTokens, styles,
                this, gfx, x, y);
    ...
    protected void paintLineNumber(Graphics gfx, Font defaultFont, int line,
                                   int x, int y) {
        ...
        if (line >= 0) {
            ...
            if (markLine == line + 1) {
                ...
                gfx.drawString(Integer.valueOf(line + 1) + "*", 0/*x-leftBorder*/, y);
            } else {
                gfx.setColor(Color.black);
                gfx.drawString(Integer.valueOf(line + 1).toString(), 0/*x-leftBorder*/, y);
            }
        }
    }

    /**
     * Paints a 80 width margin line
     */
    protected void paintMargin(Graphics gfx, Font defaultFont, int line,
                               int x, int y) {
        int height = fm.getHeight();
        gfx.setFont(defaultFont);
        int position = ((fm.charWidth(' ')) * 80) + leftBorder;
        int xPos = (position + x) - leftBorder;
        gfx.setColor(Color.LIGHT_GRAY);
        if (line >= 0) {
            //gfx.drawLine(xPos,y,xPos,height);
            y += height;
            gfx.drawString("|", xPos, y);
        }
    }
    ...
    protected void paintLineHighlight(Graphics gfx, int line, int y) {
        ...
        if (selectionStart == selectionEnd) {
            if (lineHighlight) {
                ...
                gfx.fillRect(leftBorder, y, getWidth() - leftBorder, height);
            }
        } else {
